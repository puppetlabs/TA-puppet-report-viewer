...
import modalert_puppet_run_task_investigate_helper

class AlertActionWorkerpuppet_run_task_investigate(ModularAlertBase):

    def __init__(self, ta_name, alert_name):
        super(AlertActionWorkerpuppet_run_task_investigate, self).__init__(ta_name, alert_name)
    ...
    def validate_params(self):
        ...
        if not self.get_param("bolt_investigate_target"):
            self.log_error('bolt_investigate_target is a mandatory parameter, but its value is None.')
            return False

        if not self.get_param("bolt_investigate_name"):
            self.log_error('bolt_investigate_name is a mandatory parameter, but its value is None.')
            return False
        return True
    ...
    def process_event(self, *args, **kwargs):
        ...
        try:
            if not self.validate_params():
                return 3
            status = modalert_puppet_run_task_investigate_helper.process_event(self, *args, **kwargs)
...
if __name__ == "__main__":
    exitcode = AlertActionWorkerpuppet_run_task_investigate("TA-puppet-report-viewer", "puppet_run_task_investigate").run(sys.argv)
    sys.exit(exitcode)
